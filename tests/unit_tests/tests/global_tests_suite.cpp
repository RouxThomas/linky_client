...
void run_global_tests_suite() {
	...
	//-----------------------------------------------------
	checker_int *int_check = new checker_int(int_to_str, test_report,
			is_verbose);
	...
	// register tests
	globalSuite->append(
			(void*) new tests_utils_linked_list("tests_utils_linked_list",
					int_check));
	globalSuite->append(
			(void*) new tests_utils_linked_list_char(
					"tests_utils_linked_list_char", str_check, int_check));
	globalSuite->append(
			(void*) new tests_encoder_default("tests_encoder_default",
					buffer1_for_tests,
					BUFFER_SIZE_IN_BYTE, str_check));
	globalSuite->append(
			(void*) new tests_uri_encoder("tests_uri_encoder",
					buffer1_for_tests,
					BUFFER_SIZE_IN_BYTE, int_check, str_check));
	globalSuite->append(
			(void*) new tests_uri_post_request("tests_post_request_encoder",
					buffer1_for_tests,
					BUFFER_SIZE_IN_BYTE, buffer2_for_tests,
					BUFFER_SIZE_IN_BYTE, str_check));
	globalSuite->append(
			(void*) new tests_edf_controller("tests_edf_controller",
					buffer3_for_edf,
					BUFFER_SIZE_IN_BYTE, buffer1_for_tests,
					BUFFER_SIZE_IN_BYTE, buffer2_for_tests,
					BUFFER_SIZE_IN_BYTE, str_check));
